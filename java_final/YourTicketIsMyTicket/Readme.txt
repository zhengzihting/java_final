...
2. 系統將在背景持續運作，如果要完全關閉，請在system tray的icon按右鍵開啟選單，並點選「結束程式」
3. 關於點選通知瀏覽器跳轉：這部分把提案中的想法做出來，但實作上因為把爬蟲畫面保持開啟，彈出式通知與跳轉就有點失去必要性，所以另外提供黃色按鈕可以關閉彈出式通知，對於點選通知跳轉時會開啟預設瀏覽器這方面就沒有再做優化